...
import { View, Text, TouchableOpacity, ScrollView, Image, Alert, KeyboardAvoidingView, Platform, Modal, ActivityIndicator } from "react-native";
...
export default function ModalConfiguracoes({ visible, onClose }: ModalConfiguracoesProps) {
  ...
  const campos = [
    ...
    { label: "Data de Nascimento", key: "data_nascimento", editable: true },
  ...
  const salvarEdicao = async () => {
    if (loadingSalvar) return; // previne múltiplos cliques
                ...
                {editando ? (
                  <>
                    <TouchableOpacity
                      ...
                      className="p-1"
                      disabled={loadingSalvar} // desativa clique enquanto salva
                    >
                      {loadingSalvar ? (
