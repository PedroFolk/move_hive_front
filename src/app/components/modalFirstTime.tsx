...
export default function ModalFirstTime({
  ...
  useEffect(() => {
    if (!ufCidadeJson?.estados) {
      console.error("JSON ufCidadeJson não carregado ou formato inválido");
      return;
    }
    const estadosFormatados = ufCidadeJson.estados.map((uf: any) => ({
  ...
  const handleEstadoChange = (ufSigla: string) => {
    console.log("Estado selecionado:", ufSigla);
    setEstado(ufSigla);
    setCidade("");
    ...
    if (!estadoEncontrado) {
      console.error("Estado não encontrado no JSON:", ufSigla);
      setCidades([]);
      return;
    }

    console.log("Cidades encontradas:", estadoEncontrado.cidades.length);
    const cidadesFormatadas = estadoEncontrado.cidades.map((c: string) => ({
      label: c,
      value: c,
    }));
        ...
        <SafeAreaView className="flex-1">
          {step === 1 && (
            <>
                  ...
                  <View className="flex flex-row flex-wrap justify-between">
                    {esportesList.map(({ key, label, iconType, iconName }) => {
                      ...
                      return (
                        <TouchableOpacity
                          key={key}
                          className={`w-[30%] mb-4 border rounded p-2 items-center ${esportes[key]
              ...
              >
                <Text
                  className={`text-center text-xl font-semibold ${Object.keys(esportes).length === 0
                    ? "text-white"
                    : "text-black"
                    }`}
                >
            ...
            <>
              ...
              <View className="absolute bottom-2 w-full">
                ...
                <TouchableOpacity
                  ...
                  className={`py-3 px-5 rounded-xl  border-2 mb-4 ${!cidade || !estado
